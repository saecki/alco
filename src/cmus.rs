...
fn write_autosave(cmus_scheme: impl AsRef<str>) -> anyhow::Result<()> {
    ...
    let local_path = PathBuf::from(tilde(&format!("~/.config/cmus/{}.theme", cmus_scheme.as_ref())).to_string());
    ...
    let new_scheme: Vec<_> = new_scheme_str.lines().filter(|&s| s.starts_with("set color_")).collect();
    ...
    for l in original_autosave_str.lines() {
        if l.starts_with("set color_") {
            ...
            if let Some(var) = l.split('=').next() {
                println!("var: {}", var);
                let new = new_scheme.iter().find(|&s| s.starts_with(var)).and_then(|s| s.split('=').skip(1).next());
                println!("new: {:?}", new);
